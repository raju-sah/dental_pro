            ...
            <div class="card-body">
                ...
                <x-form.wrapper action="{{ route('admin.services.store') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <x-form.textarea label="Description" id="description" name="description"
                        value="{{ old('description') }}" rows="6" cols="6" />
                    ...
                    <x-form.enum-select label="Discount Type" :options="\App\Enums\ServiceType::cases()" :req="true" col="6"
                        name="service_type"></x-form.enum-select>

                        <x-form.checkbox label="Status" id="status" name="status" value="1" class="form-check-input"
                        isEditMode="yes" :isChecked="true" />
