    ...
    <div class="container-xxl">
        ...
        <div class="card">
            <div class="card-body">
                ...
                <x-form.wrapper action="{{ route('admin.services.update', $service->id) }}" method="POST"
                    ...
                    <x-form.textarea label="Description" id="description" name="description"
                        value="{{ $service->description }}" rows="5" cols="5" />


                    <x-form.enum-select label="Service Type" col="4" :req="true" :model="$service->service_type" :options="\App\Enums\ServiceType::cases()"
                        name="service_type"></x-form.enum-select>




                    <x-form.checkbox label="Status" id="status" name="status" value="1" class="form-check-input"
                        isEditMode="yes" :isChecked="$service->status ? 'checked' : ''" />
