    ...
    <div class="card">
        <div class="card-body">
            ...
            <x-form.wrapper action="{{route('admin.seo-settings.update', $seo_setting->id)}}" method="POST" enctype="multipart/form-data">
                @method('PATCH')
                <x-form.row>
                <x-form.input type="text" col="6" :req="true" label="Site title" id="site_title" name="site_title" value="{{$seo_setting->site_title}}" />
                <x-form.input type="text" col="6" :req="true" label="Home title" id="home_title" name="home_title" value="{{$seo_setting->home_title}}" />
                </x-form.row>
                <x-form.row>
                <x-form.input type="text" col="6" label="Site description" id="site_description" name="site_description" value="{{$seo_setting->site_description}}" />
                <x-form.input type="text" col="6" label="Keyword" id="keyword" name="keyword" value="{{$seo_setting->keyword}}" />
                </x-form.row>
                <x-form.input type="text" label="Google analytics" id="google_analytics" name="google_analytics" value="{{$seo_setting->google_analytics}}" />
