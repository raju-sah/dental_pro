            ...
            <div class="card-body">
                ...
                {{-- @if (isset($setting->id))/ --}}
                    {{-- <x-form.wrapper action="{{ route('admin..update', $setting->id) }}" method="POST"
                        ...
                        <x-form.wrapper action="{{$setting ? route('admin.settings.update', $setting->id) : route('admin.settings.store')}}" method="POST" enctype="multipart/form-data">
                @method('PATCH')
                <xform.row>
                    <x-form.input type="file" label="Image" id="image" name="image" alt="image"
                        accept="image/*" onchange="previewThumb(this,'featured-thumb')" />
                </xform.row>
                <x-form.preview id="featured-thumb" . url="{{ $setting->image_path }}" />
                <x-form.row>
